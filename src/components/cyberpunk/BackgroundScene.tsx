...
function ParticleField() {
  ...
  return (
    <points>
      ...
      <pointsMaterial size={0.15} sizeAttenuation transparent opacity={0.75} blending={THREE.AdditiveBlending} vertexColors depthWrite={false} />
    </points>
  )
}
...
function SceneCore() {
  ...
  useEffect(() => {
    const cam = new THREE.PerspectiveCamera(60, size.width / size.height, 0.1, 200)
    ...
    composer.addPass(new UnrealBloomPass(new THREE.Vector2(size.width, size.height), 0.5, 0.3, 0.3))
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: -1, pointerEvents: 'none' }}>
      <Canvas gl={{ antialias: true }} frameloop="always">
        ...
        <directionalLight intensity={0.5} color="#02D7F2" position={[0, 20, 10]} />
        <Grid
          cellColor="#02D7F2"
          sectionColor="#25E1ED"
          args={[120, 120]}
        ...
        {buildings.map((b, i) => (
          <mesh key={i} position={b.position}>
            ...
            <meshStandardMaterial color="#0A0A0F" emissive={b.emissive} emissiveIntensity={0.5} />
          </mesh>
        ))}
        {shards.map((s, i) => (
          <Float key={i} speed={s.speed} floatIntensity={s.intensity} floatingRange={[-0.3, 0.3]} rotationIntensity={1}>
            <mesh position={s.position}>
              <octahedronGeometry args={[s.size]} />
              <meshStandardMaterial color={s.color} emissive={s.color} emissiveIntensity={1.5} wireframe />
            </mesh>
          </Float>
        ))}
        <SceneCore />
