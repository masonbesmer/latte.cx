...
export const crateItems = (() => {
  const items: Array<{ type: 'divider'; data: CrateDivider } | { type: 'service'; data: Service }> = [];
  const categories: ServiceCategory[] = [
    'Infrastructure', 'Media', 'Gaming', 'Home', 'Development', 'Monitoring',
  ];
...
export function getStatusColor(status: ServiceStatus): string {
  switch (status) {
    ...
    case 'down':    return 'var(--status-red)';
  }
}
